...
-(void)setModel:(EssenceListModel *)model{
    ...
    if (height > ZMSCREENH) {
     ...
     [self.imageView sd_setImageWithURL:[NSURL URLWithString:model.image2] placeholderImage:[UIImage imageNamed:@"post_placeholderImage"] options:SDWebImageHighPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
- (IBAction)repostBtn:(id)sender {
     ...
     [SVProgressHUD showErrorWithStatus:@"需要登录才能转发哦！"];
}
